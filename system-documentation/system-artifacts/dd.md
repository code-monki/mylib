...
This draft starts the component decomposition work. Detailed subsections will be added component-by-component. Each component section must define responsibilities, non-responsibilities, invariants, interfaces, data contracts, failure behavior, NFR derivation, and test intent before implementation can be authorized.
...
Test case IDs remain **TBD** until the test plan is drafted.
...
![Dd Server Host Internal Structure](img/dd-server-host-internal-structure.svg)

Source: [`img-src/dd-server-host-internal-structure.mmd`](img-src/dd-server-host-internal-structure.mmd "Dd Server Host Internal Structure Mermaid Source")
...
| Operator runbook depth (cert issuance/renewal/trust bootstrap) | **Partial** | Policy set in DD; step-by-step operator docs remain required under NFR-004 documentation work |
...
These IDs are DD-level placeholders and SHALL map to canonical test-plan IDs when the formal test plan is published.
...
| ------------- | ------------------- | ------------------ |
| UI stack | **HLA-SHELL** | Framework/library choice, accessibility strategy, theme implementation, packaging implications |
...
| Search engine model | **HLA-SEARCH** | **Baseline selected:** SQLite FTS. **Deferred refinement:** external multi-user path (Meilisearch first candidate; OpenSearch/Tantivy-based options), schema/migration and operational controls |
| OCR engine model | **HLA-OCR**, **HLA-BOUND-OCR** | **Baseline selected:** Tesseract + Leptonica with OpenCV preprocessing and Poppler extraction path; viewer-side PDF UX candidate PDFium. **Deferred refinement:** exact pipeline profiles, quality thresholds, and reproducibility controls |
| Security library choices | **HLA-SECURITY** | **Baseline selected:** Argon2id + OpenSSL + **`jwt-cpp`** (+ nlohmann/json). **Deferred refinement:** concrete Argon2 binding package, secure random helper standardization, and exact `jwt-cpp`/OpenSSL algorithm matrix + version pins per platform |
| Security policy defaults | **HLA-SECURITY** | Exact password symbol set, session timeout defaults/bounds, per-user concurrent-session behavior, throttling defaults/bounds |
...
Failure semantics to define before DD approval:
...
Undefined failure behavior blocks DD approval.
...
Test planning is not yet authorized as a completed phase, but DD must define test intent before implementation:
...
- DD SHALL NOT lock per-OS installer implementation details in this pass (for example package format/tool choice, service-registration scripts, installer UI flow, notarization/signing pipeline mechanics, or platform-specific post-install scripting).
...
- DD artifact exists as **v0.1 draft**.
- Initial DD component decomposition exists for **HLA-SHELL** through **HLA-RELEASE** (**§4.1–§4.12**), with refinement still in progress.
- RTM DD Artifact references are populated at section level; implementation/test/packaging/orchestration columns remain **TBD**.

Traceability gaps block DD approval.
...
| ---- | ----- | ---------------------- |
| UI stack fails accessibility or packaging needs | Moderate | Compare candidate stacks against FR-022, FR-024, NFR-006, packaging impact |
...
- Which UI framework/library satisfies accessibility, theme, packaging, and cross-platform constraints?
- What certificate and trust-anchor workflow should remote deployments use, including public CA / Let's Encrypt / ACME documentation vs operator-managed private CA?
- Search-engine transition trigger is partially set: move beyond SQLite FTS when observed performance no longer meets deployment needs/SLOs; what exact quantitative thresholds, load shapes, and measurement protocol should gate adoption (Meilisearch first candidate; OpenSearch/Tantivy-based options later)?
- What exact OCR/extraction pipeline profiles (OpenCV stages, quality thresholds, retry/defer rules) should ship for reproducibility and operator control?
- Which packaging/orchestration choices are required by DD dependencies?

Unresolved high-impact issues block DD approval.
...
- All components decomposed? **Partial** — initial decomposition present for **§4.1–§4.12**; refinement still required  
- Interface contracts complete? **No**  
- NFR derivation explicit? **Partial** — required derivations identified, not completed  
- Deterministic-probabilistic refinement complete? **No**  
- Traceability mapping complete? **No** — RTM DD/test/package columns remain TBD  
- Human approval granted? **No**  

**Action:** Remain in **Detailed Design**. Implementation remains blocked.
...
Role:  
Date (YYYY-MM-DD):  
Version Incremented: No (draft **v0.1** scaffold; increment on future approved DD revisions per change control)

---

End of Detailed Design (draft)
